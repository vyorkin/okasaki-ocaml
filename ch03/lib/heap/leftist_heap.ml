...
module Mk (E : Ordered.S) : Heap_intf.S = struct
  ...
  let mkT (x, l, r) =
    if rank l >= rank r then T (rank l + 1, x, l, r) else T (rank r + 1, x, r, l)
  ...
  let findMin = function E -> None | T (_, x, _, _) -> Some x
  let deleteMin = function E -> None | T (_, _, l, r) -> Some (merge (l, r))
end
